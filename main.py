...
#!/usr/bin/env python3
# ================================================================
#  Diffusion‑based Code‑Generator — extended edition  ✧ v4 ✧
# ================================================================
...
class DiffusionTransformer(nn.Module):
    """
    * DECODER-based architecture for cross-attention.
    * self‑conditioning and prompt-conditioning (CFG) ready.
    ...
    def forward(self, noisy_emb: torch.Tensor, t: torch.Tensor,
        ...
        # The decoder takes tgt (what we're generating) and memory (what we're conditioning on)
        x = self.decoder(
            tgt=tgt,
            memory=memory,
...
def load_checkpoint(ckpt_path: Path, model, optimizer, scheduler, ema, device, is_ddp: bool):
...
def train(model, opt, scheduler, epochs, steps_ep, loader, acp, ema, device, amp, sampler_cfg, is_ddp, ckpt_dir: str, args: argparse.Namespace, tokenizer: Tokenizer, pad_id: int):
    ...
    if args.resume and rank == 0:
        ...
        if latest_ckpt:
            start_epoch, _ = load_checkpoint(latest_ckpt, model, opt, scheduler, ema, device, is_ddp)
    ...
    for ep in range(start_epoch, epochs + 1):
        ...
        for step in range(steps_ep):
            ...
            tgt_pad_mask = (x_tokens == pad_id)
            prompt_pad_mask = (prompt_tokens == pad_id)
            B, L = x_tokens.size()
            ...
            with torch.amp.autocast("cuda", enabled=use_cuda_amp):
                ...
                eps1, x0_1, logits1 = model(noisy_emb, t, tgt_pad_mask, None, current_prompt_emb, current_prompt_mask)
